    ...
    def silhouette_score(self, X: np.ndarray) -> float:
        ...
        for i in range(n):
            ...
            if same_cluster.sum() > 1:
                dd = same_cluster.copy()
                dd[i] = False
                a = np.mean(np.linalg.norm(X[i] - X[dd], axis=1))
            else:
                a = 0
...
because 'california' has more dimentions
...
'''Let's check the accuracy of my code and compare it with sklearn model of kmeans'''
from sklearn.cluster import KMeans as SKKMeans
from sklearn.metrics import silhouette_score

sk = SKKMeans(n_clusters=3, random_state=42)
